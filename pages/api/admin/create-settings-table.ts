...
async function handler(req: NextApiRequest, res: NextApiResponse, user: { id: string; email: string; securityLevel?: string }) {
  try {
    // Only allow POST for table creation
    if (req.method !== 'POST') {
      return res.status(405).json({
        success: false,
        error: {
          code: 'METHOD_NOT_ALLOWED',
          message: 'Only POST method is allowed for table creation',
          details: null
        },
    ...
    // Log admin table creation action
    console.log(`Admin settings table creation initiated`, {
      adminId: user.id,
    ...
    if (error) {
      console.error('Admin settings table creation error:', error);
    ...
    console.error('Admin settings table creation API error:', error);
    return res.status(500).json({
      success: false,
      error: {
...
export default withErrorHandler(withCMSSecurity(handler, {
  ...
  auditAction: 'settings_table_created'
}));
